...
import re
# We'll split safely using the signature length
target = text.split('@router.post("/analyze"')[1].split('# --------------------------', 1)[1]

new_content = text.split('@router.post("/analyze"')[0] + BACKGROUND_TASK_CODE + "\n# --------------------------" + target
